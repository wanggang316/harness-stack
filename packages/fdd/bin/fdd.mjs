...
var FeatureStatusSchema = external_exports.enum(featureStatuses);
var terminalFeatureStatuses = /* @__PURE__ */ new Set(["completed", "cancelled"]);
var FeatureSchema = external_exports.object({
  id: external_exports.string().regex(KEBAB, "feature id must be kebab-case"),
  description: external_exports.string().min(1),
  agent: external_exports.string().min(1),
  // the subagent/skill that implements this feature
  milestone: external_exports.string().min(1),
  preconditions: external_exports.array(external_exports.string()).default([]),
  expectedBehavior: external_exports.array(external_exports.string()).default([]),
  verificationSteps: external_exports.array(external_exports.string()).default([]),
  fulfills: external_exports.array(external_exports.string().regex(VAL_ID, "fulfills entries must be VAL-<AREA>-NNN ids")).default([]),
  status: FeatureStatusSchema.default("pending")
}).strict();
var FeaturesFileSchema = external_exports.object({
  features: external_exports.array(FeatureSchema).default([])
}).strict();
var assertionStatuses = ["pending", "passed", "failed", "blocked"];
var AssertionStatusSchema = external_exports.enum(assertionStatuses);
var AssertionSchema = external_exports.object({
  status: AssertionStatusSchema,
  evidence: external_exports.string().min(1).optional()
  // free-form pointer: path / url / note
}).strict();
var ValidationStateFileSchema = external_exports.object({
  assertions: external_exports.record(external_exports.string(), AssertionSchema).default({})
}).strict();
var SuccessStateSchema = external_exports.enum(["success", "partial", "failure"]);
var DiscoveredIssueSchema = external_exports.object({
  summary: external_exports.string(),
  severity: external_exports.enum(["blocker", "bug", "tech-debt", "nit"]).optional(),
...
var HandoffSchema = external_exports.object({
  ...
  verificationEvidence: external_exports.array(external_exports.string()).default([]),
  discoveredIssues: external_exports.array(DiscoveredIssueSchema).default([]),
  whatWasLeftUndone: external_exports.array(external_exports.string()).default([]),
  criticalContext: external_exports.array(external_exports.string()).default([]),
  returnToController: external_exports.boolean().default(false)
}).strict();
var SealedMilestonesSchema = external_exports.object({
  sealed: external_exports.array(external_exports.string()).default([])
}).strict();
...
function plansDir(env = process.env) {
  return join(runtimeRoot(env), "plans");
}
function activeFilePath(env = process.env) {
  return join(plansDir(env), ".active");
}
function planDir(slug, env = process.env) {
  return join(plansDir(env), slug);
}
var featuresPath = (dir) => join(dir, "features.json");
var statePath = (dir) => join(dir, "validation-state.json");
var contractPath = (dir) => join(dir, "validation-contract.md");
var sealedPath = (dir) => join(dir, "sealed-milestones.json");
var handoffPath = (dir, featureId) => join(dir, "handoffs", `${featureId}.json`);
async function readActiveSlug(env = process.env) {
  const f = activeFilePath(env);
  if (!existsSync(f)) return void 0;
  const slug = (await readText(f)).trim();
  return slug.length > 0 ? slug : void 0;
}
async function writeActiveSlug(slug, env = process.env) {
  const { mkdir: mkdir3, writeFile: writeFile2 } = await import("node:fs/promises");
  await mkdir3(plansDir(env), { recursive: true });
  await writeFile2(activeFilePath(env), `${slug}
`, "utf8");
}
async function resolvePlan(explicitSlug, env = process.env, mustExist = true) {
...
async function listPlanSlugs(env = process.env) {
...
async function saveFeatures(dir, file) {
...
async function saveState(dir, file) {
  await writeJsonAtomic(statePath(dir), file);
}
async function loadSealed(dir) {
...
async function saveSealed(dir, file) {
  await writeJsonAtomic(sealedPath(dir), file);
}

// src/core.ts
var ASSERTION_HEADING = /^###\s+(VAL-[A-Z0-9]+-\d{3})\b/;
function parseAssertionIds(markdown) {
  const ids = [];
  const seen = /* @__PURE__ */ new Set();
...
function findNextPending(features) {
  return features.find((f) => f.status === "pending");
}
function withStatus(features, id, status) {
...
async function readContractIds(dir) {
  const path = contractPath(dir);
  if (!existsSync2(path)) throw new PlanError("data", `validation-contract.md not found at ${path}`);
...
async function setAssertion(dir, id, status, evidence) {
  const state = await loadState(dir);
  state.assertions[id] = evidence !== void 0 ? { status, evidence } : { status };
  await saveState(dir, state);
}
async function gate(dir) {
  const { assertions } = await loadState(dir);
  const entries = Object.entries(assertions);
  const failing = entries.filter(([, a]) => a.status !== "passed").map(([id, a]) => ({ id, status: a.status }));
  return { passed: entries.length > 0 && failing.length === 0, failing, total: entries.length };
}
async function contractCoverage(dir) {
...
async function writeHandoff(dir, featureId, source) {
  const result = HandoffSchema.safeParse(source);
  if (!result.success) {
    throw new PlanError("usage", `invalid handoff JSON:
${formatIssues2(result.error.issues)}`);
  }
  const path = handoffPath(dir, featureId);
  await writeJsonAtomic(path, result.data);
  return path;
}
async function readHandoff(dir, featureId) {
  const path = handoffPath(dir, featureId);
  if (!existsSync3(path)) throw new PlanError("data", `no handoff recorded for ${featureId} at ${path}`);
...
function requirePositional(positional, index, name) {
  ...
  if (v === void 0 || v.length === 0) throw new UsageError(`<${name}> is required`);
  return v;
}
...
async function runInit(parsed, out) {
...
async function runUse(parsed, out) {
  const slug = requirePositional(parsed.positional, 0, "slug");
  if (!existsSync4(planDir(slug))) throw new PlanError("data", `plan "${slug}" not found at ${planDir(slug)}`);
  await writeActiveSlug(slug);
  out.write(`active plan -> ${slug}
`);
  return 0;
}
async function runActive(out) {
  const slug = await readActiveSlug();
  if (slug === void 0) throw new PlanError("data", "no active plan; run `fdd use <slug>`");
  out.write(`${slug}	${planDir(slug)}
`);
  return 0;
}
async function runListPlans(out) {
...
async function runNextFeature(parsed, out) {
  const f = await nextFeature(await dirFor(parsed.flags));
  if (f !== void 0) out.write(`${f.id}	${f.agent}	${f.milestone}
`);
  return 0;
}
async function runSetStatus(parsed, out) {
...
async function runListFeatures(parsed, out) {
...
async function runMilestoneStatus(parsed, out) {
  ...
  for (const status of featureStatuses) {
    ...
    if (n !== void 0 && n > 0) out.write(`${status}	${n}
`);
  }
  return 0;
}
async function runInitState(parsed, out) {
  const n = await initState(await dirFor(parsed.flags));
  out.write(`validation-state.json regenerated (${n} assertions)
`);
  return 0;
}
async function runContractCoverage(parsed, out) {
...
async function runSetAssertion(parsed, out) {
...
async function runGate(parsed, out) {
...
async function runSealMilestone(parsed, out) {
  const milestone = requirePositional(parsed.positional, 0, "milestone");
...
async function runIsSealed(parsed, out) {
  const milestone = requirePositional(parsed.positional, 0, "milestone");
  out.write(await isSealed(await dirFor(parsed.flags), milestone) ? "yes\n" : "no\n");
  return 0;
}
async function runWriteHandoff(parsed, out) {
...
async function runHandoff(parsed, out) {
  const featureId = requirePositional(parsed.positional, 0, "feature-id");
...
var isCliEntrypoint = (() => {
  if (processArgv[1] === void 0) return false;
  try {
