...
`harness-stack:pr` opens the PR; `harness-stack:land` brings it home. Watch CI, handle review feedback, keep mergeability, then squash-merge.
...
For a one-tick check (no persistent loop), use `/harness-stack:pr-watch` instead.
...
1. **Locate the PR**
   ...
   If no PR exists for the branch, stop — open one with `harness-stack:pr` first.

2. **Pre-flight**
   - Working tree clean. If not, commit pending work via `/harness-stack:commit`.
...
3. **Conflict check**
   - If `mergeable == "CONFLICTING"` or `mergeStateStatus == "DIRTY"`:
     - Sync the branch — see [git/references/sync.md](../git/references/sync.md) (rebase) or [pull.md](../git/references/pull.md) (merge), depending on project policy.
     - Force-push only if history was deliberately rewritten ([push.md](../git/references/push.md)).
...
5. **React**
   ...
   | CI check failed (real) | Pull logs, fix locally, commit via `/harness-stack:commit`, push, go back to step 4. |
   ...
   | Review comment outstanding | Hand off to `harness-stack:review-receive`. Don't merge until acknowledged and (if needed) addressed. |
   | Push rejected (auth / permission / policy) | Surface error. Do not paper over — see [git/references/push.md](../git/references/push.md). |
...
Full depth: `harness-stack:review-receive`.
...
| Yield to the user while the PR is still actionable | The whole point of `harness-stack:land` is to drive to merge. Stop only when blocked. |
