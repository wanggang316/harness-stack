...
The example has mock agents (so the smoke path works without any keys) plus templates for Anthropic and OpenAI-compatible providers and one CLI agent. Tell the user which environment variables the api providers expect (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, etc.) — they need to be set before any api agent will work.
