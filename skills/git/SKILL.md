...
本技能讲的是**心法**——原则、约定、反模式。各项的完整规范见 `references/`。
...
让某条 worktree 分支追上 trunk 时，不必切回主 worktree——直接对最新的 `origin/main` rebase（或 merge）即可，见 [references/update-from-main.md](references/update-from-main.md)（命令 `/harness-stack:sync-main`）。
...
| 主题 | 参考 |
|---|---|
| Commit 规范 | [references/commit.md](references/commit.md) |
| 同步（rebase + push） | [references/sync.md](references/sync.md) |
| Pull / merge 更新分支 | [references/pull.md](references/pull.md) |
| 用 main 更新 worktree 分支 | [references/update-from-main.md](references/update-from-main.md) |
