...
This skill covers the **mindset** — principles, conventions, anti-patterns. Operational depth (how to commit, sync, pull, push) lives in `references/`. Day-to-day entry points are the slash commands `/harness-stack:commit` and `/harness-stack:git-sync`.
...
Each worktree is an isolated checkout — agents can work in parallel without branch switching. If an experiment fails, delete the worktree, nothing is lost. See `harness-stack:env-init` for per-worktree runtime isolation.
...
|---|---|---|
| Create a commit | [references/commit.md](references/commit.md) | `/harness-stack:commit` |
| Sync (rebase + push) | [references/sync.md](references/sync.md) | `/harness-stack:git-sync` |
