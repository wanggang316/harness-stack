...
reviews     approves    harness-stack:planner
```
...
`docs/user-tests/<feature>.md`, authored separately by /harness-stack:test-spec. Do NOT
...
- Acceptance criteria capture *intent*; the runnable, observable form lives in `docs/user-tests/<feature>.md`, authored separately by `/harness-stack:test-spec`
...
- `/harness-stack:design` for technical design decisions (if multiple approaches exist)
- `/harness-stack:test-spec` for the user-test set (cases binding to AC IDs)
- `/harness-stack:planner` for creating the execution plan
- `/harness-stack:exec-plan` for implementation

`/harness-stack:design` and `/harness-stack:test-spec` can run in parallel — they are independent views (engineering vs QA) of the same approved spec.
...
- [ ] Acceptance criteria capture intent only — no implementation references; runnable assertions deferred to `/harness-stack:test-spec`
