...
**When NOT to use:** No plan exists yet (use `harness-stack:planner` first), or the change is a single-file fix with obvious scope.

**vs `harness-stack:team`:** Use `harness-stack:team` when the plan has 3+ tasks, the tasks are well-specified enough to dispatch to fresh-context implementers, and you want full subagent automation (controller does not write code, every task gets spec + code review gates). Use `harness-stack:exec-plan` (this skill) when the plan is smaller, the work is exploratory, or the controller needs to keep hands on the keyboard for in-flight decisions.
