...
**这是「流程外」评审路径。** `harness-stack:feature-driven-development` 内部已对每个 feature 自动 code-review、每个 milestone 跑 scrutiny——**走 FDD 时不需要本技能**。review-request 用于 **FDD 之外** 的改动：手改、hotfix、未走 FDD 的工作，或想对一个 PR 做一次独立审视。
...
- 打开或合并 PR 之前。
- 完成一项 **FDD 之外** 的非平凡改动（手改、hotfix、重构）之后。
...
- FDD 内部已自动 code-review（每 feature）+ scrutiny（每 milestone）——**流程内不需要本技能**。
- 仅当你在 FDD 之外手动改了它的产物，才对那段改动单独 review。
...
组装一份精简 brief 作为 subagent 的 user-message——只放下面这些**输入**；评审方法、severity 与 Output Format 都在 `agents/code-reviewer.md` 的 system prompt 里：
