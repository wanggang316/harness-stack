...
**When NOT to use：** 产品已清晰且稳定。要构建一个 feature，用 `/harness-stack:fdd`。需要技术设计文档时，用 `/harness-stack:design`。
...
- **harness-stack:fdd** 构建单个 feature → 按 plan 维度的状态存于 `.harness-runtime/plans/<slug>/`
