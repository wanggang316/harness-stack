...
| **Build** (main flow) | Any non-trivial change | `harness-stack:feature-driven-development` — contract-first: plan → validation-contract → features → milestone-gated execution loop. Uses `harness-stack:test-spec` (Phase 2) and `harness-stack:user-test` (Phase 4) internally. `harness-stack:tdd` (test-first) inside an implementer's task. |
