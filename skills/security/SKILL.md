...
> **与 FDD 的分工：** 深度威胁建模由 `harness-stack:security-auditor` 子代理执行。在 `feature-driven-development` 流程内，它会在 milestone 边界、当 diff 触及敏感面（auth/secrets/crypto/裸查询/shell/依赖升级）时**自动并行派发**。本技能是 **FDD 之外** 的独立手动安全审计与加固路径（就像 `review-request` 之于 code-review）。
